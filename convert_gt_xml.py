...
xml_list = [f for f in os.listdir('../Annotations') if f.endswith('xml') or f.endswith('xml2')]
if len(xml_list) == 0:
  print("Error: no .xml files found in Annotations")
  sys.exit()
for tmp_file in xml_list:
  print(tmp_file)
  with open(os.path.join('../results/groundtruth', tmp_file.replace(".xml", ".txt") if tmp_file.endswith('xml') else tmp_file.replace(".xml2", ".txt")), "a") as new_f:
